...
const CatalogPage = () => {
  ...
  const movies = Array.from({ length: moviesPerPage });

  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  // Función para navegar a la descripción
  const handleMovieClick = (id) => {
    // Aquí podrías pasar el id real: navigate(`/pelicula/${id}`)
              ...
              <div className="flex flex-col overflow-hidden">
                    ...
                    return (
                      ...
                      >
                        ...
                        <h4 className={`mt-2 text-[11px] min-[480px]:text-sm font-bold transition-colors duration-500 truncate
                          ...
                          Contenido {((currentPage - 1) * moviesPerPage) + idx + 1}
                        </h4>
                ...
                <div className="mt-16 bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col min-[600px]:flex-row items-center justify-between gap-6 shadow-xl">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => handlePageClick(currentPage - 1)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-black/40 border border-white/10 text-gray-400 hover:text-[#3b82f6] active:scale-90 transition-all cursor-pointer">
                      ...
                      {getPaginationGroup().map((page) => (
                        <button
                          ...
                          className={`w-10 h-10 rounded-xl font-bold text-sm transition-all border cursor-pointer active:scale-90 ${
                            currentPage === page ? 'bg-[#3b82f6] border-[#3b82f6] text-white shadow-lg shadow-[#3b82f6]/20' : 'bg-black/40 border-white/10 text-gray-400'
                          }`}
                        >
                          {page}
                        </button>
