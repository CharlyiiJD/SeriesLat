...
import { Play, Eye, AlertTriangle, Server, User, ChevronRight } from 'lucide-react';
import 'swiper/css';
...
const TabOnline = () => {
  ...
  return (
    <div
      className="animate-in fade-in duration-500 space-y-0 min-[801px]:space-y-10"
      onClick={() => setActiveGlobalId(null)}
    >

      {/* --- REPRODUCTOR --- */}
      <div className="space-y-0 min-[801px]:space-y-4">
        <div className="w-full aspect-video bg-black min-[801px]:rounded-2xl border-y min-[801px]:border border-white/10 flex items-center justify-center relative overflow-hidden shadow-2xl shadow-black/60">
        ...
        <div className="flex flex-col gap-3 px-4 pt-4 min-[801px]:px-0 min-[801px]:pt-0">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 bg-white/[0.02] p-3 min-[801px]:p-4 min-[801px]:rounded-2xl border-y min-[801px]:border border-white/5 mx-0 px-4 min-[801px]:px-4">
          <div className="flex items-center gap-2 min-[801px]:gap-3 flex-wrap">
            <span className="text-[8px] min-[801px]:text-[10px] font-bold text-gray-500 uppercase">
              Fuente: <span className="text-[#3b82f6] ml-1">{servidores[servidorActivo].label}</span>
            </span>
            <button className="flex items-center gap-1.5 text-red-500/80 hover:text-red-500 text-[8px] min-[801px]:text-[10px] font-bold uppercase transition-colors cursor-pointer">
              <AlertTriangle size={11} className="min-[801px]:w-[14px] min-[801px]:h-[14px]" /> Reportar fallo
            </button>
          </div>
          <div className="flex items-center gap-1.5 min-[801px]:gap-2 text-gray-400 font-bold text-[8px] min-[801px]:text-xs bg-white/5 px-2.5 min-[801px]:px-4 py-1.5 min-[801px]:py-2 rounded-xl border border-white/5">
            <Eye size={12} className="min-[801px]:w-4 min-[801px]:h-4 text-[#3b82f6]" />
      ...
      {/* --- ELENCO --- */}
      <div className="px-4 pt-8 min-[801px]:px-0 min-[801px]:pt-0">
        <div className="flex items-center justify-between mb-8 gap-3">
          ...
          {reparto.length > 6 && (
            <button
              ...
              className="flex items-center gap-1 bg-[#3b82f6] hover:bg-[#2563eb] text-white px-3 min-[801px]:px-4 py-1.5 min-[801px]:py-2 rounded-xl text-[8px] min-[801px]:text-[10px] font-bold uppercase tracking-wider transition-all whitespace-nowrap flex-shrink-0"
            >
              {verTodoReparto ? 'Ver menos' : 'Ver todo'}
              <ChevronRight size={10} className={`min-[801px]:w-3 min-[801px]:h-3 transition-transform ${verTodoReparto ? 'rotate-90' : ''}`} />
            </button>
          )}
        </div>
        <div className="grid grid-cols-3 min-[801px]:grid-cols-4 md:grid-cols-6 gap-6">
          {repartoMostrar.map((actor, i) => (
            <div
              key={i}
              className="group flex flex-col items-center cursor-pointer"
              onClick={(e) => { e.stopPropagation(); navigate('/actor/tom-holland'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
            >
              <div className="relative w-20 h-20 min-[801px]:w-24 min-[801px]:h-24 md:w-28 md:h-28 mb-3 overflow-hidden rounded-full border-2 border-white/5 group-hover:border-[#3b82f6] transition-all duration-500 shadow-xl">
              ...
              <p className="text-center font-bold text-[10px] min-[801px]:text-[11px] text-white/90 group-hover:text-[#3b82f6] transition-colors truncate w-full">{actor.nombre}</p>
              <p className="text-center text-[8px] min-[801px]:text-[9px] text-gray-500 uppercase mt-1 tracking-tighter">{actor.personaje}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* --- RELACIONADOS --- */}
      <div className="px-4 pt-8 pb-8 min-[801px]:px-0 min-[801px]:pt-6 min-[801px]:pb-0">
              ...
              return (
                <SwiperSlide key={idx}>
                  <div
                    ...
                    onClick={(e) => { e.stopPropagation(); navigate('/pelicula/descripcion'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} // 👈 stopPropagation agregado
