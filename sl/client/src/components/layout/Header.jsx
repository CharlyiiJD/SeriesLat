...
import RegisterModal from '../ui/RegisterModal'; // Importación necesaria
...
const Header = () => {
  ...
  const [isRegisterOpen, setIsRegisterOpen] = useState(false); // Nuevo estado para registro
      ...
      <header className={`fixed top-0 w-full z-50 transition-transform duration-300 ease-out ${visible ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        <div className={`absolute top-0 left-0 w-full h-[92px] bg-black z-[100] transition-all duration-500 ease-in-out flex items-center px-6 gap-3 ${isSearchOpen && isMobile ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
          <div className="relative flex-1">
            ...
            />
            <IoIosSearch className="absolute right-3 top-1/2 -translate-y-1/2 text-xl text-gray-400" />
          </div>
        ...
        <div className={`px-6 py-6 relative z-[60] transition-all duration-500 ${isScrolled ? 'bg-[#0c1821] backdrop-blur-md shadow-lg py-4' : 'bg-gradient-to-b from-black/95 to-transparent'}`}>
          <div className="max-w-7xl mx-auto flex items-center justify-between text-white relative">
            ...
            {/* CENTRO: Nav PC o Buscador Tablet */}
            <div className="flex-1 flex justify-center items-center px-4">
              {isDesktop && (
                <nav className="flex items-center gap-8 text-[15px] font-medium tracking-wide">
                  <a href="#" className="hover:text-[#3b82f6] transition cursor-pointer">Inicio</a>
                  <a href="#" className="hover:text-[#3b82f6] transition cursor-pointer">Películas</a>
                  <a href="#" className="hover:text-[#3b82f6] transition cursor-pointer">Series</a>
                  <div className="relative" ref={menuRef}>
                    <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center gap-2 transition-colors cursor-pointer ${isOpen ? 'text-[#3b82f6]' : 'hover:text-[#3b82f6]'}`}>
                      Categorías {isOpen ? <IoIosArrowUp /> : <IoIosArrowDown />}
                    </button>
                    {isOpen && (
                      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-6 w-[320px] bg-zinc-900/95 backdrop-blur-xl border border-white/10 p-4 rounded-xl shadow-2xl z-50">
                        <div className="grid grid-cols-2">
                          {categorias.map((cat, index) => (
                            <a key={index} href="#" className="text-gray-300 hover:text-[#3b82f6] hover:bg-white/5 transition-colors text-[13px] py-2.5 px-4 border-b border-white/[0.05] border-r border-white/[0.05] last:border-b-0 cursor-pointer">{cat}</a>
                          ))}
                        </div>
                      </div>
                    )}
                  </div>
                  <a href="#" className="hover:text-[#3b82f6] transition cursor-pointer">Anime</a>
                </nav>
              )}
              ...
              {isTablet && (
                <div className="flex items-center w-full max-w-[95%] relative">
                  ...
                  />
                  <IoIosSearch className="absolute right-4 text-xl text-gray-400" />
                </div>
              )}
            </div>

            {/* DERECHA: Buscador / Ingresar / Burger */}
              ...
              {isDesktop && (
                <>
                  <div className="flex items-center group relative">
                    ...
                    />
                    <IoIosSearch className="absolute right-3 text-xl text-gray-400 group-focus-within:text-[#3b82f6] transition-colors" />
                  </div>
                  <button 
        ...
        <div className={`fixed top-[92px] right-0 h-[calc(100vh-92px)] w-[60%] sm:w-[40%] bg-zinc-900/98 backdrop-blur-2xl transition-transform duration-500 ease-in-out z-[55] border-l border-white/5 ${isBurgerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <nav className="flex flex-col p-8 text-white text-lg gap-6 font-light overflow-y-auto h-full overscroll-contain">
            <a href="#" className="hover:text-[#3b82f6] border-b border-white/5 pb-2 cursor-pointer">Inicio</a>
            <a href="#" className="hover:text-[#3b82f6] border-b border-white/5 pb-2 cursor-pointer">Películas</a>
            <a href="#" className="hover:text-[#3b82f6] border-b border-white/5 pb-2 cursor-pointer">Series</a>
            
            <div>
              <button onClick={() => setIsCatExpanded(!isCatExpanded)} className="flex items-center justify-between w-full hover:text-[#3b82f6] pb-2 cursor-pointer">
              ...
              <div className={`grid grid-cols-1 gap-3 overflow-hidden transition-all duration-500 ${isCatExpanded ? 'max-h-[500px] mt-4 opacity-100' : 'max-h-0 opacity-0'}`}>
                {categorias.map((cat, i) => (
                  <a key={i} href="#" className="text-sm text-gray-400 hover:text-[#3b82f6] pl-2 cursor-pointer">{cat}</a>
                ))}
              </div>
            </div>
            
            <a href="#" className="hover:text-[#3b82f6] cursor-pointer">Anime</a>
